...
## D12 — Open catalogue needs an INSERT policy too, not just SELECT

The original RLS migration gave `skills` and `resources` a SELECT policy for
`authenticated` and stopped there. But `profileService.findOrCreateSkill` /
`findOrCreateResource` — the whole mechanism behind "Add a skill we missed"
(spec §14) and the open-catalogue design in `docs/architecture.md` — need to
INSERT a new row whenever a user types something outside the seeded 28
skills / 13 resources. With no INSERT policy, RLS silently blocked every
such insert, and the client swallowed the resulting error with no message
(separately fixed — both call sites now surface `AppError.message` instead
of failing silently). Net effect: anyone who described a skill not already
in the catalogue looked "stuck" with no explanation. Fixed in migration
`20260915120000_open_catalogue_inserts.sql`, which grants INSERT on both
tables to any authenticated user (`with check (true)`) — matching the SELECT
policy's same "any signed-in user" scope, since this is intentionally an
open, growable catalogue, not a curated one.

## D13 — Disable Supabase's email-confirmation requirement for this prototype

Local dev (`supabase/config.toml`, generated by `supabase init`) already
defaults to `enable_confirmations = false`. A freshly created **hosted**
Supabase project defaults to the opposite — confirmation required — which
two things follow from directly:

1. `authService.signUp` establishes a session immediately in local dev, but
   on an unconfirmed hosted project the user is created without an active
   session, so the app's own `redirect('/onboarding/welcome')` right after
   registering runs into the proxy's auth guard and bounces straight back to
   `/login` — looking exactly like "registration doesn't work," for a
   different reason than the RLS bug above.
2. Every signup attempt sends a confirmation email through Supabase's
   built-in (very low, free-tier) send rate limit — easy to exhaust during
   normal testing or a live demo with several stakeholders registering in
   the same window, at which point signups start failing with 429s.

Per spec §41 ("do not add... identity verification... unless specifically
approved") and the demo-reliability priority in spec §32/§49, the hosted
project's **Authentication → Sign In / Providers → Email → "Confirm email"**
